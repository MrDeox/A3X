...
DEFAULT_MODEL_PATH = "../models/mistral-7b-instruct-v0.1.Q4_K_M.gguf" # Ajuste se necessário
...
def find_and_kill_server(server_exe_name="llama-server", sig=signal.SIGTERM):
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            ...
            if proc_name == server_exe_name or proc.info['name'] == server_exe_name:
                print(f"  Encontrado processo '{server_exe_name}' (PID: {proc.pid}). Tentando terminar com sinal {sig.name}...")
    ...
    if killed_count > 0:
        print(f"  {killed_count} processo(s) '{server_exe_name}' sinalizado(s) para terminar.")
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
             try:
                 ...
                 if proc_name == server_exe_name or proc.info['name'] == server_exe_name:
                      print(f"  Processo '{server_exe_name}' (PID: {proc.pid}) ainda vivo. Forçando KILL.")
                      proc.kill()
                      force_killed +=1
             except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
                 pass
        if force_killed > 0:
             print(f"  {force_killed} processo(s) '{server_exe_name}' forçados a terminar (KILL).")
             time.sleep(1)

    return killed_count + force_killed > 0 # Retorna True se algum processo foi encontrado/morto
...
def start_server(server_path, model_path, ngl, host, port, context_size=2048):
    ...
    command = [
        ...
        "--host", host,
        "--port", str(port),
        "-c", str(context_size), # Tamanho do contexto
    ...
    for i in range(attempts):
        try:
            ...
            if response.status_code == 200 or response.status_code == 404:
                print(f"  Servidor respondeu (Status: {response.status_code}). Pronto para testes.")
                return True
            else:
                 print(f"  Tentativa {i+1}/{attempts}: Servidor respondeu com status inesperado {response.status_code}.")
        ...
        if i < attempts - 1:
             time.sleep(timeout // attempts + 1) # Espera um pouco antes de tentar novamente
...
def run_benchmark(chat_url, prompt, num_runs=3, api_timeout=180):
    ...
    for i in range(num_runs):
        ...
        try:
            response = requests.post(chat_url, headers=headers, json=payload, timeout=api_timeout)
            ...
            return None, None # Indica falha
             ...
             print(f"      ERRO: Falha na conexão com a API (Servidor caiu?).")
             return None, None # Indica falha
        except requests.exceptions.RequestException as e:
            ...
            if response is not None and 500 <= response.status_code < 600:
                 print("      ERRO 5xx: Possível erro de Out-of-Memory (OOM) no servidor.")
            return None, None # Indica falha
    ...
    print(f"  Benchmark concluído para este ngl.")
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark llama-server com diferentes valores de -ngl.")
    parser.add_argument("--server-path", type=str, default=DEFAULT_SERVER_EXE,
    ...
    parser.add_argument("--ngl-values", type=str, required=True,
                        help='Valores de NGL a testar. Ex: "0,8,16,24,33" ou "16-28"')
    parser.add_argument("--host", type=str, default="127.0.0.1", help="Host do servidor (default: 127.0.0.1)")
    parser.add_argument("--port", type=int, default=8080, help="Porta do servidor (default: 8080)")
    parser.add_argument("--warmup-seconds", type=int, default=25,
                        help="Tempo (s) para esperar o servidor carregar (default: 25)")
    parser.add_argument("--test-prompt", type=str, default=DEFAULT_TEST_PROMPT,
    ...
    if not os.path.isfile(args.server_path):
        print(f"ERRO CRÍTICO: Executável do servidor não encontrado em '{args.server_path}'")
        sys.exit(1)
    ...
    for ngl in ngl_values_to_test:
        ...
        try:
            server_process = start_server(args.server_path, args.model_path, ngl, args.host, args.port, args.context_size)

            print(f"  Aguardando {args.warmup_seconds}s para o servidor iniciar e carregar o modelo...")
            ...
            try:
                 if server_process.poll() is not None: # Se poll() não for None, o processo terminou
                      print(f"  ERRO: Processo do servidor (PID: {server_process.pid}) terminou inesperadamente durante o warmup (Código: {server_process.poll()}). Provável OOM.")
                      results[ngl] = "Falha (Crash no Warmup - OOM?)"
                      continue # Pula para o próximo NGL
            except Exception as e:
                 print(f"  AVISO: Não foi possível verificar o status do processo do servidor: {e}")
            ...
            avg_time, std_dev_time = run_benchmark(chat_url, args.test_prompt, args.num_runs, args.api_timeout)
            ...
            if server_process and server_process.poll() is None:
                 find_and_kill_server(server_exe_name, sig=signal.SIGKILL) # Força kill para garantir limpeza rápida
            else:
