...
def mock_embedding_functions(mocker):
    ...
    mock_get_recall = mocker.patch(
        "skills.memory.recall.get_embedding", return_value=MOCK_EMBEDDING_ARRAY
    )
...
async def test_save_memory_success(mock_db_connection, mock_embedding_functions):
    ...
    # mock_get_embedding_recall = mock_embedding_functions  # F841
    ...
    with patch("skills.memory.save.get_embedding") as mock_get_embedding_save:
        ...
        )  # INSERT semantic, CHECK vss, INSERT vss
        mock_cursor.execute.assert_any_call(
            mock.ANY, ("Test content", MOCK_EMBEDDING_BLOB, '{"source": "test"}')
        ...
        mock_cursor.execute.assert_any_call(
            mock.ANY, (123, MOCK_EMBEDDING_BLOB)
...
@pytest.mark.asyncio
async def test_save_memory_duplicate_content(
    mock_db_connection, mock_embedding_functions
):
    ...
    assert (
        result["data"]["vss_updated"] is True
    )  # Mesmo duplicado, VSS foi atualizado/verificado
    ...
    )  # INSERT semantic(IGNORE), SELECT ID, CHECK vss, INSERT vss
    mock_cursor.execute.assert_any_call(
        mock.ANY, ("Duplicate content", MOCK_EMBEDDING_BLOB, None)
    )  # Check main INSERT call args
    mock_cursor.execute.assert_any_call(
        "SELECT id FROM semantic_memory WHERE content = ?", ("Duplicate content",)
    )  # Check SELECT ID call
    mock_cursor.execute.assert_any_call(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='vss_semantic_memory'"
    )  # Check VSS table existence check
    mock_cursor.execute.assert_any_call(
        mock.ANY, (456, MOCK_EMBEDDING_BLOB)
    )  # Check VSS INSERT call args
    mock_conn.commit.assert_called_once()
    mock_conn.close.assert_called_once()
...
@pytest.mark.asyncio
async def test_save_memory_missing_content(
    mock_db_connection, mock_embedding_functions
):
...
@pytest.mark.asyncio
async def test_save_memory_embedding_error(
    mock_db_connection, mock_embedding_functions
):
...
@pytest.mark.asyncio
async def test_recall_memory_vss_not_exists(
    mock_db_connection, mock_embedding_functions
):
...
@pytest.mark.asyncio
async def test_recall_memory_missing_query(
    mock_db_connection, mock_embedding_functions
):
...
@pytest.mark.asyncio
async def test_recall_memory_embedding_error(
    mock_db_connection, mock_embedding_functions
):
...
async def test_recall_memory_db_error(mock_db_connection, mock_embedding_functions):
    ...
    mock_cursor.fetchone.side_effect = [("vss_semantic_memory",), (1,)]
    ...
    assert "Erro inesperado ao buscar memória: VSS search failed" in result.get(
        "data", {}
    ).get("message", "")
