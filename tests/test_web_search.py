...
def test_skill_search_web_success(mocker, mock_tavily_client):
    ...
    mock_client.search.assert_called_once_with(
        query="teste de busca", search_depth="basic", max_results=5
    )
    assert result["status"] == "success"
    assert result["action"] == "web_search_completed"
    assert (
        "Busca na web (Tavily) por 'teste de busca' concluída"
...
def test_skill_search_web_success_max_results(mocker, mock_tavily_client):
    ...
    mock_client.search.return_value = (
        MOCK_TAVILY_SUCCESS_RESPONSE  # A resposta mock não precisa mudar
    )
    ...
    mock_constructor.assert_called_once_with(api_key="fake_key")
    mock_client.search.assert_called_once_with(
        query="test query", search_depth="basic", max_results=3
    )  # Verifica max_results
    assert result["status"] == "success"
    assert result["action"] == "web_search_completed"
    assert (
        "Busca na web (Tavily) por 'test query' concluída" in result["data"]["message"]
    )
...
def test_skill_search_web_api_error(mocker, mock_tavily_client):
    ...
    mock_constructor.assert_called_once_with(api_key="fake_key")
    mock_client.search.assert_called_once_with(
        query="error query", search_depth="basic", max_results=5
    )
    assert result["status"] == "error"
    assert result["action"] == "web_search_failed"
    assert (
        "Erro ao executar a busca na web (Tavily): Tavily Connection Error"
        in result["data"]["message"]
    )
...
def test_skill_search_web_disabled(mocker):
    ...
    mock_constructor = mocker.patch(
        "skills.web_search.TavilyClient"
    )  # Mock para garantir que NÃO seja chamado
    ...
    assert (
        result["status"] == "error"
    )  # <<< CORRIGIDO: Deve ser error quando desabilitado >>>
    assert result["action"] == "web_search_disabled"
    assert (
        "Busca na web (Tavily) está desabilitada na configuração"
        in result["data"]["message"]
    )
...
def test_skill_search_web_no_api_key(mocker):
    ...
    assert (
        "Chave da API Tavily (TAVILY_API_KEY) não encontrada."
        in result["data"]["message"]
    )
