...
async def test_react_agent_run_handles_parsing_error(
    ...
    assert reflector_call_args.get('action_name') == '_parse_llm' # Internal action for parsing
    assert reflector_call_args.get('observation_dict', {}).get('status') == 'error'
    assert reflector_call_args.get('observation_dict', {}).get('action') == 'parsing_failed'
...
async def test_react_agent_run_handles_max_iterations(
    ...
    mock_executor = mocker.patch('core.tool_executor.execute_tool', return_value=list_success_result)
...
async def test_react_agent_run_handles_failed_planning(
    ...
    mock_planner = mocker.patch('core.planner.generate_plan', new_callable=AsyncMock, return_value=None)
