...
async def test_react_agent_run_list_files(
    ...
    with (
        patch("core.agent.call_llm", new_callable=AsyncMock) as mock_call_llm,
        patch("core.tool_executor.execute_tool", new=mock_list_files_tool),
        patch(
            "core.planner.generate_plan", new_callable=AsyncMock
        ...
        agent_instance.agent_logger = (
            agent_logger  # Attach logger if not done automatically
        )
        ...
        mock_generate_plan.assert_called_once_with(
            objective, tool_desc, agent_instance.agent_logger, agent_instance.llm_url
        )
        ...
        final_answer_content = json.loads(LLM_JSON_RESPONSE_LIST_FILES_FINAL)[
            "action_input"
        ]["answer"]
        assert final_response_dict is not None
        assert (
            final_answer_content in final_response_dict
        )  # Check if the final answer is in the returned string
...
async def test_react_agent_run_handles_tool_execution_error(
    ...
    # Use AsyncMock
    mock_planner = mocker.patch(
        "core.planner.generate_plan", new_callable=AsyncMock, return_value=expected_plan
    )
    ...
    mock_tool_executor = mocker.patch(
        "core.agent.tool_executor.execute_tool", return_value=EXECUTE_CODE_RESULT_ERROR
    )
