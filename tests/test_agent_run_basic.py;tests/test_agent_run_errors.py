...
@pytest.mark.asyncio
async def test_react_agent_run_list_files(agent_instance, mock_db, mocker, 
LLM_JSON_RESPONSE_LIST_FILES, LLM_JSON_RESPONSE_LIST_FILES_FINAL, LIST_FILES_RESULT_JSON):
...
@pytest.mark.asyncio
async def test_react_agent_run_final_answer_direct(agent_instance, mock_db, mocker, LLM_JSON_RESPONSE_HELLO_FINAL):
...
async def test_react_agent_run_handles_llm_call_error(
    ...
    mock_reflector = mocker.patch('core.agent.agent_reflector.reflect_on_observation', new_callable=AsyncMock)
    ...
    assert "Erro: Falha ao comunicar com LLM" in final_response # Agent stops on LLM error before reflector decision matters here
...
async def test_react_agent_run_handles_tool_execution_error(
    agent_instance, mock_code_tools, mock_db, mocker,
    LLM_JSON_RESPONSE_EXECUTE_FAILING_CODE, EXECUTE_CODE_RESULT_ERROR_JSON
):
    ...
    mock_execute_func = MagicMock(return_value=json.loads(EXECUTE_CODE_RESULT_ERROR_JSON))
