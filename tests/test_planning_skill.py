...
async def test_hierarchical_planner_success(mock_call_llm):
    ...
    mock_llm_response_json = json.dumps(["Step 1: Do this", "Step 2: Do that", "Step 3: Finish up"])
    mock_call_llm.return_value = mock_llm_response_json # <<< MODIFIED: Use return_value
    ...
    expected_prompt = build_planning_prompt(objective, tools_desc, DEFAULT_PLANNER_SYSTEM_PROMPT)
...
async def test_hierarchical_planner_invalid_json(mock_call_llm):
    ...
    mock_call_llm.return_value = mock_llm_response_text # <<< MODIFIED: Use return_value
...
async def test_hierarchical_planner_not_list(mock_call_llm):
    ...
    assert "Invalid plan structure: Parsed JSON is type 'dict', not list." in result["data"]["message"]
