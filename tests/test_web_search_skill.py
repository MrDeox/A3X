...
from a3x.skills.web_search import web_search # Moved to top import block
...
async def test_web_search_skill_returns_results(mock_ddgs):
    ...
    mock_ddgs_instance.text.return_value = [
        ...
        {
            "title": "Official Website of Paris",
            "href": "https://www.paris.fr/en",
            "body": "The official website for the City of Paris.",
        },
    ]
    ...
    assert (
        first_result["url"] == "https://en.wikipedia.org/wiki/Paris"
    )  # Check 'url' (mapped from 'href')
    assert (
        first_result["snippet"]
        == "Paris is the capital and most populous city of France..."
    )  # Check 'snippet' (mapped from 'body')

    # Verify the mocked DDGS().text() was called correctly
    mock_ddgs_instance.text.assert_called_once_with(
        query, max_results=expected_max_results
    )
