...
def test_modify_code_success_add_comment(mock_post):
    ...
    assert call_kwargs.get("headers") == LLAMA_DEFAULT_HEADERS # Verifica headers padrão
...
def test_modify_code_success_markdown_cleanup(mock_post):
    ...
    assert result.get("status") == "success"
    data = result.get("data", {})
    assert data.get("modified_code") == MODIFIED_CODE_COMMENT # Código correto após limpeza
...
def test_modify_code_missing_modification():
    ...
    data = result.get("data", {})
    assert "Erro: A instrução de modificação (modification) não foi especificada." in data.get("message", "")
...
def test_modify_code_missing_code():
    ...
    assert result.get("action") == "modify_code_failed"
    data = result.get("data", {})
    assert "Erro: O código a ser modificado (code_to_modify) não foi fornecido." in data.get("message", "")
...
def test_modify_code_request_exception(mock_post):
    ...
    assert result.get("action") == "modify_code_failed"
    data = result.get("data", {})
    assert "Erro de comunicação com o servidor LLM: Network Error" in data.get("message", "")
...
def test_modify_code_http_error(mock_post):
    ...
    assert "Erro de comunicação com o servidor LLM: 500 Server Error" in data.get("message", "")
...
def test_modify_code_unexpected_llm_format(mock_post):
    ...
    mock_response = MagicMock()
    mock_response.json.return_value = MOCK_LLM_RESPONSE_UNEXPECTED_FORMAT # Formato inválido
    mock_response.raise_for_status = MagicMock()
    mock_post.return_value = mock_response
