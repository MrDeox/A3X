...
)  # OU sys.executable se rodar pytest de dentro do venv
...
def run_a3x_test_capture(
    input_file: str = None, command: str = None, timeout: int = 240
) -> tuple[int, str, str]:  # Timeout aumentado para 240s
    """
    ...
    if input_file:
        ...
        )  # Usar project_root para input file
    ...
    try:
        ...
        print(
            f"[Pytest] Comando concluído com código: {return_code}"
        )  # Log do resultado
        ...
        pytest.fail(
            f"Timeout ({timeout}s) excedido ao executar: {' '.join(cmd)}\nStdout:\n{stdout}\nStderr:\n{stderr}"
        )
...
def test_react_gen_mod_exec():
    ...
    assert (
        "Observação: Código Python" in stdout and "executado com sucesso." in stdout
    ), "Msg sucesso execute_code não encontrada (substring check)."
...
def test_react_search_web():
    ...
    assert (
        "Observação: Busca por" in stdout
        and "concluída com" in stdout
        and "resultado(s)." in stdout
    ), "Observação de conclusão de busca não encontrada (substring check)."
...
def managed_llama_server(llama_server_port):  # Use session scope
    ...
    model_path = os.path.join(
        project_root, "models", "test-model.gguf"
    )  # Path to a dummy model file
    ...
    if not os.path.exists(server_script) or not os.access(server_script, os.X_OK):
        pytest.skip(
            f"Mock server executable not found or not executable: {server_script}"
        )
    ...
    try:
        ...
        if server_process.poll() is not None:
            ...
            pytest.fail(
                f"Mock server failed to start. Exit code: {server_process.returncode}\nCmd: {' '.join(cmd)}\nStdout:\n{stdout}\nStderr:\n{stderr}"
            )

        print(
            f"[Pytest Fixture] Mock server started successfully (PID: {server_process.pid})"
        )
        ...
        if server_process and server_process.poll() is None:
            ...
            try:
                ...
                print(
                    "[Pytest Fixture] Server did not terminate gracefully, killing..."
                )
                server_process.kill()
                server_process.wait()
...
def test_react_list_files(managed_llama_server):  # Added fixture
    ...
    assert "Observação: Nenhum arquivo" in stdout or (
        "Observação:" in stdout and "arquivo(s)" in stdout and "encontrado(s)" in stdout
    ), "Observação de listagem não encontrada (substring check)."

    # Simplifica asserção da resposta final
    assert "[A³X]: Nenhum arquivo" in stdout or (
        "[A³X]:" in stdout and "arquivo(s)" in stdout and "encontrado(s)" in stdout
    ), "Resposta final sobre listagem não encontrada (substring check)."

    print("  >> Resposta final encontrada (verificada por substrings)")
