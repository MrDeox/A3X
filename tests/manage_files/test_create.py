...
from skills.manage_files import create_file, WORKSPACE_ROOT
...
async def test_create_file_success(mock_validate, tmp_path, mock_path_object):
    ...
    resolved_path_mock.exists.return_value = False # Ensure exists is False for creation
    ...
    def decorator_side_effect(*args, **kwargs):
        ...
        else: raise ValueError("Could not find original function")
    ...
    with patch('builtins.open', new_callable=MagicMock) as mock_open:
        ...
        result = create_file(content=content_to_write, overwrite=False, resolved_path=resolved_path_mock, original_path_str=original_path)
    ...
    mock_open.assert_called_once_with(str(resolved_path_mock._real_path), "w", encoding="utf-8")
...
async def test_create_file_overwrite_success(mock_validate, tmp_path, mock_path_object):
    ...
    with patch('builtins.open', new_callable=MagicMock) as mock_open:
        ...
        result = create_file(content=content_to_write, overwrite=True, resolved_path=resolved_path_mock, original_path_str=original_path)
    ...
    mock_open.assert_called_once_with(str(resolved_path_mock._real_path), "w", encoding="utf-8")
    mock_open().__enter__().write.assert_called_once_with(content_to_write)
    assert resolved_path_mock._real_path.read_text() == content_to_write # Verify new content

@pytest.mark.asyncio
@patch('skills.manage_files.validate_workspace_path')
async def test_create_file_exists_no_overwrite(mock_validate, tmp_path, mock_path_object):
...
@pytest.mark.asyncio
@patch('skills.manage_files.validate_workspace_path')
async def test_create_file_target_is_directory(mock_validate, tmp_path, mock_path_object):
