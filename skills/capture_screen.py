...
def skill_capture_screen(action_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with mss.mss() as sct:
            if region:
                ...
                if not all(k in region for k in ("top", "left", "width", "height")):
                     raise ValueError("A região deve conter 'top', 'left', 'width', 'height'.")
                ...
                if monitor_index < 1 or monitor_index >= len(sct.monitors):
                     logger.warning(f"Índice de monitor inválido ({monitor_index}). Usando monitor primário (1).")
    ...
    except FileNotFoundError: # mss pode levantar isso se algo estiver errado com monitores?
        logger.error("Erro ao encontrar monitores ou problema com mss.")
        return {"status": "error", "action": "capture_failed", "data": {"message": "Erro interno ao acessar informações de tela/monitores."}}
    except (ValueError, KeyError) as e: # Erro em parâmetros (e.g., região mal formada)
         logger.error(f"Erro nos parâmetros fornecidos: {e}")
         return {"status": "error", "action": "capture_failed", "data": {"message": f"Erro nos parâmetros: {e}"}}
