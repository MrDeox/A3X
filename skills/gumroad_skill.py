...
from core.tools import skill # CHANGED: Import 'skill' decorator from core.tools
...
@skill(
    ...
    parameters={
        ...
        "files": (List[str], "List of relative file paths (within workspace) to include in the product.")
        # Add other relevant parameters like permalink, options, etc. later
    }
)
def create_product(name: str, description: str, price: float, files: List[str], agent_history: list | None = None) -> Dict[str, Any]:
    logger.info(f"Executing skill: create_product (simulation) for '{name}'")
    if not GUMROAD_API_KEY:
        return create_skill_response(status="error", action="gumroad_api_error", error_details="Gumroad API Key not configured.", message="Gumroad API Key is missing.")

    # --- Simulation Logic ---
    print(f"[SIMULATION] Called create_product with name: {name}, price: ${price:.2f}, files: {files}")
    simulated_product_id = f"sim_prod_{name.replace(' ', '_')[:10].lower()}_{int(datetime.now().timestamp()) % 10000}"
...
def list_products(agent_history: list | None = None) -> Dict[str, Any]:
    ...
    simulated_products = [
        {"id": "sim_prod_checklist_1234", "name": "Checklist de automação com IA local", "price": 2.99, "published": True},
        {"id": "sim_prod_prompt_pa_5678", "name": "Prompt Pack para criadores de conteúdo", "price": 4.99, "published": True},
        {"id": "sim_prod_template__9012", "name": "Template Notion para organização pessoal", "price": 9.99, "published": False}, # Example unpublished
    ]
...
@skill(
    ...
    parameters={
        "product_id": (str | None, "Optional: Filter sales data for a specific product ID."),
        "period": (str | None, "Optional: Time period (e.g., '7d', '30d', 'all').")
    }
)
def get_sales_data(product_id: str | None = None, period: str | None = None, agent_history: list | None = None) -> Dict[str, Any]:
    logger.info(f"Executing skill: get_sales_data (simulation) for product '{product_id or 'all'}'")
    if not GUMROAD_API_KEY:
        return create_skill_response(status="error", action="gumroad_api_error", error_details="Gumroad API Key not configured.", message="Gumroad API Key is missing.")

    # --- Simulation Logic ---
    print(f"[SIMULATION] Called get_sales_data for product_id: {product_id}, period: {period}")
    # Reuse logic from sales_fetcher for simulation consistency
    from skills.sales_fetcher import SalesFetcherSkill # Import locally for simulation
    fetcher = SalesFetcherSkill()
    simulated_response = fetcher.fetch_sales_data(product_id=product_id)
    simulated_sales_records = simulated_response.get("data", {}).get("sales_records", [])
