...
def skill_ocr_image(action_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not image_path_str:
        return {
            "status": "error",
            "action": "ocr_failed",
            "data": {"message": "Error: 'image_path' parameter is required."},
        }
    ...
    try:
        ...
        if not _is_path_safe(image_path):
            return {
                ...
                "action": "ocr_failed",
                "data": {
                    "message": f"Error: Accessing image outside allowed directories is not permitted: {image_path_str}"
                },
            }
        ...
        if not image_path.is_file():
            return {
                "status": "error",
                "action": "ocr_failed",
                "data": {"message": f"Error: Image file not found at: {image_path}"},
            }
        ...
        logger.error(
            "Tesseract executable not found. Make sure Tesseract OCR is installed and in your system's PATH."
        )
        return {
            "status": "error",
            "action": "ocr_failed",
            "data": {
                "message": "Error: Tesseract executable not found. Please install Tesseract OCR."
            },
        }
        ...
        logger.error(
            f"File not found during OCR process, potentially resolved path issue for: {image_path_str}"
        )
        return {
            "status": "error",
            "action": "ocr_failed",
            "data": {
                "message": f"Error: Image file not found (path resolution issue?): {image_path_str}"
            },
        }
    except Exception as e:
        ...
        return {
            "status": "error",
            "action": "ocr_failed",
            "data": {"message": f"Unexpected error during OCR: {e}"},
        }
