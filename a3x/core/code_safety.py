...
def is_safe_ast(code_string: str) -> tuple[bool, str]:
    ...
    try:
        ...
        allowed_nodes = (
            ...
            ast.Dict,
            ast.Subscript,
            ast.Index,  # Para acesso a listas/dicionários
        )
        allowed_calls = {
            "print",
            "len",
        ...
        # allowed_imports = set()  # F841 - Nenhum import permitido por padrão
        ...
        for node in ast.walk(tree):
                ...
                if isinstance(node.func, ast.Name):
                    if node.func.id not in allowed_calls:
                        ...
                        if not is_defined_locally:
                            return (
                                False,
                                f"Chamada de função não permitida: {node.func.id}",
                            )
                # Bloquear chamadas de atributos (ex: obj.method()) por segurança inicial
                elif isinstance(node.func, ast.Attribute):
                    return (
                        False,
                        f"Chamada de método/atributo não permitida: {ast.dump(node.func)}",
                    )
            # Bloquear imports
            ...
            if isinstance(node, ast.Attribute):
                ...
                if node.attr.startswith(
                    "_"
                ):  # Bloqueia atributos "privados" ou "mágicos"
            ...
            if isinstance(node, ast.BinOp):
                if not isinstance(node.op, (ast.Add, ast.Sub, ast.Mult, ast.Div)):
                    return (
                        False,
                        f"Operação binária não permitida: {type(node.op).__name__}",
                    )
