...
from a3x.core.config import TAVILY_API_KEY
...
def web_search(query: str, max_results: int = 5) -> dict: # Added max_results parameter
    ...
    try:
        ...
        with DDGS() as ddgs:
             raw_results = list(ddgs.text(query, max_results=max_results)) # Get results as list
        ...
        if raw_results:
            ...
            logger.info(f"DuckDuckGo search successful, found {len(search_results)} results.")
        else:
        ...
        return {
            ...
            "action": "web_search_failed",
            "data": {
                "message": f"Web search failed due to an error: {e}"
            },
        }
