...
def _get_device_id() -> int:
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            logger.info(
                "GPU (CUDA/ROCm) detectada. Usando GPU para classificação de sentimento."
            )
            return 0  # Use the first available GPU
    except ImportError:
        logger.warning(
            "PyTorch não encontrado. Classificação de sentimento será executada na CPU."
        )
    except Exception as e:
        logger.error(f"Erro ao verificar GPU com PyTorch: {e}. Usando CPU.")

    logger.info(
        "Nenhuma GPU compatível detectada ou PyTorch ausente. Usando CPU para classificação de sentimento."
    )
...
def _load_pipeline_internal():
    ...
    try:
        ...
        logger.info(
            f"[Sentiment] Pipeline '{MODEL_NAME}' carregado em {load_duration:.2f} segundos no dispositivo '{device_name}'."
        )

    except ImportError:
        logger.error(
            "[Sentiment] Biblioteca 'transformers' não encontrada. Instale com 'pip install transformers[torch]' ou 'transformers[tensorflow]'."
        )
        _pipeline_loading_error = ImportError("transformers not found")
    except Exception as e:
        logger.exception(
            "[Sentiment] Erro inesperado ao carregar o pipeline de sentimento:"
        )
...
def skill_classify_sentiment(action_input: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not text_content or not isinstance(text_content, str):
        return {
            "status": "error",
            "action": "sentiment_classification_failed",
            "data": {
                "message": "Error: 'text' parameter is required and must be a string."
            },
        }
    ...
    if _pipeline_instance is None and _pipeline_loading_error is None:
        logger.debug(
            "[Sentiment] Tentando carregar pipeline em skill_classify_sentiment."
        )
        _load_pipeline_internal()

    if _pipeline_loading_error is not None:
        logger.error(
            f"[Sentiment] Pipeline não pôde ser carregado anteriormente: {_pipeline_loading_error}"
        )
        return {
            "status": "error",
            "action": "sentiment_classification_failed",
            "data": {
                "message": f"Failed to load sentiment analysis model: {_pipeline_loading_error}"
            },
        }
    if _pipeline_instance is None:
        ...
        return {
            ...
            "action": "sentiment_classification_failed",
            "data": {
                "message": "Sentiment analysis model instance is unexpectedly None."
            },
        }
    ...
    try:
        logger.info(
            f"[Sentiment] Classificando sentimento para texto (length: {len(text_content)} characters)..."
        )
        start_time = time.time()
        ...
        end_time = time.time()
        logger.info(
            f"[Sentiment] Classificação concluída em {end_time - start_time:.4f} segundos. Resultado: {label}, Score: {score:.4f}"
        )
        ...
        if label and isinstance(label, str):
            try:
                sentiment_rating = int(
                    label.split()[0]
                )  # Pega o número antes de ' stars'
            except (ValueError, IndexError):
                logger.warning(
                    f"[Sentiment] Não foi possível extrair a pontuação numérica do label: '{label}'"
                )
        ...
        logger.exception(
            "[Sentiment] Erro inesperado durante a classificação de sentimento:"
        )
        return {
            "status": "error",
            "action": "sentiment_classification_failed",
            "data": {
                "message": f"Unexpected error during sentiment classification: {e}"
            },
        }
