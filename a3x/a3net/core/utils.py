...
try:
    Path(OUTPUT_LOG_FILE).parent.mkdir(parents=True, exist_ok=True)
except Exception as e:
    logger.error(f"Error creating directory for log file {OUTPUT_LOG_FILE}: {e}")
...
def append_to_log(message: str):
    """Appends a message line to the session output log file."""
    try:
        with open(OUTPUT_LOG_FILE, "a", encoding="utf-8") as f:
            timestamp = datetime.datetime.now().strftime("%H:%M:%S.%f")[:-3]
            f.write(f"# [{timestamp}] {message}\n")
    except Exception as e:
        logger.error(f"Failed to append to log file {OUTPUT_LOG_FILE}: {e}")
...
# Example function that might have previously used the imported handle_directive
# Now it would need the handler and memory_bank passed to it, or it should
# interact via the message queue.
async def process_some_util_task(data: Dict, 
                                 # memory_bank: MemoryBank, # Needs to be passed in
                                 # directive_handler: callable # Needs to be passed in
                                 ):
    logger.info(f"Processing util task with data: {data}")
    # Example placeholder logic
    if data.get("action") == "reflect_on_error":
        error_context = data.get("context", "Unknown error")
        # Previously might have done:
        # directive = {"type": "reflect_fragment", "fragment_id": "error_reflector", "context": error_context}
        # await handle_directive(directive, memory_bank, ...)
        
        # New approach: Post message or call passed handler
        logger.warning("process_some_util_task needs refactoring to use passed handler or message queue instead of direct handle_directive call.")
        # Example (if message posting is appropriate):
        # await post_message_to_queue(
        #      message_type="a3l_command", 
        #      content=directive, 
        #      target_fragment="Executor"
        # )
        return {"status": "needs_refactoring", "message": "Util function needs update"}
    
    return {"status": "success", "message": "Util task processed (placeholder)"} 