...
async def _execute_actual_plan_step(agent, step_objective: str, context: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    """
    logger.info(f"Executing Step Objective: '{step_objective}' using base ReactAgent logic...")
    try:
        ...
        async for result_chunk in agent.run(objective=step_objective):
             ...
             elif isinstance(result_chunk, dict) and result_chunk.get("status"): # Check for final status dict
                 final_step_result = result_chunk
             # else: process intermediate thoughts/actions if needed

        if not final_step_result:
             logger.warning(f"Base agent run for step '{step_objective}' finished without yielding a final result dict or string.")
             final_step_result = {"status": "error", "message": "Step execution finished with unknown state."}
        ...
        agent.add_history_entry("assistant", memory_content) # Use agent's history method
        logger.info(f"Step '{step_objective}' execution completed. Status: {final_step_result.get('status')}")
        return final_step_result

    except Exception as e:
        logger.exception(f"Exception during execution of step '{step_objective}':")
        error_result = {"status": "error", "message": f"Exception during step execution: {e}"}
        # Store error in short-term memory
        memory_content = f"Execution result for '{step_objective}': error - Exception: {e}"
        agent.add_history_entry("assistant", memory_content)
        return error_result


async def execute_plan_with_reflection(agent, plan: List[str], context: Dict[str, Any]) -> AsyncGenerator[Dict[str, Any], None]:
    """
    ...
    """
    step_index = 0
    max_steps = len(plan) * 2 # Limit steps to prevent infinite loops if modification keeps failing
    ...
    while step_index < len(plan) and executed_step_count < max_steps:
        ...
        logger.info(f"--- Processing Step {step_index + 1}/{len(plan)}: {current_step_objective[:80]}... ---")
        ...
        reflection_input = {
            ...
            "simulated_outcome": simulation.get("simulated_outcome", "Simulation failed or missing outcome."),
            "context": context
        }
        # Default reflection outcome in case of error calling the tool
        step_reflection = {"status": "error", "decision": "skip", "justification": "Default skip due to reflection error calling tool"}
        try:
            ...
            logger.exception(f"Error executing reflect_plan_step tool for step {step_index}: {current_step_objective}")
            step_reflection["justification"] = f"Reflection tool execution failed: {reflect_err}"

        yield {"type": "pre_reflection", "step_index": step_index, "content": step_reflection}
        ...
        if step_decision == "execute":
            logger.info(f"--- Executing Step {step_index + 1}/{len(plan)} based on reflection: {current_step_objective[:80]}... ---")
            step_result = await _execute_actual_plan_step(agent, current_step_objective, context)
            step_index += 1 # Move to next step only on successful execution or skip

        elif step_decision == "skip":
            logger.info(f"--- Skipping Step {step_index + 1}/{len(plan)} based on reflection: {current_step_objective[:80]}... Reason: {step_justification} ---")
            step_result = {"status": "skipped", "action": "step_skipped", "data": {"message": f"Step skipped due to reflection. Reason: {step_justification}"}}
            step_index += 1 # Move to next step

        elif step_decision == "modify":
            logger.warning(f"--- Plan Modification Required for Step {step_index + 1} based on reflection: {current_step_objective[:80]}... Reason: {step_justification} ---")
            yield {"type": "modification_trigger", "step_index": step_index, "reason": step_justification}

            # Signal back to the agent/caller that replanning is needed.
            logger.error("Replanning required but not implemented within execution_logic. Signaling modification and skipping step.")
            step_result = {"status": "skipped", "action": "step_skipped_modification_required", "data": {"message": f"Modification required but replanning must be handled by caller. Reason: {step_justification}"}}
            step_index += 1 # Skip the problematic step to avoid loop within this function

        else: # Unknown decision
            logger.error(f"--- Skipping Step {step_index + 1}/{len(plan)} due to unknown reflection decision ('{step_decision}'): {current_step_objective[:80]}... Reason: {step_justification} ---")
            step_result = {"status": "skipped", "action": "step_skipped_unknown_decision", "data": {"message": f"Step skipped due to unknown reflection decision: {step_decision}. Reason: {step_justification}"}}
            step_index += 1 # Move to next step

        if step_result:
            execution_results.append(step_result)
            yield {"type": "execution_step", "step_index": step_index - 1 if step_decision != "modify" else step_index, "result": step_result}
        else:
             logger.error(f"No step result generated for step index {step_index}. This indicates a logic error.")
             execution_results.append({"status": "error", "action": "internal_loop_error", "data": {"message": "No result for step."}})
             step_index += 1 # Prevent infinite loop

    logger.info(f"Finished executing plan steps. Total steps processed in loop: {executed_step_count}")
