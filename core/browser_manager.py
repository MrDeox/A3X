        ...
        async with self._lock:
            if self._playwright_context is None:
                try:
                    ...
                    logger.critical(f"Failed to start Playwright context: {e}", exc_info=True)
            ...
            async with self._lock:
                ...
                if self._browser is None:
                    if self._playwright_context is None:
                         ...
                         raise BrowserManagerError("Playwright context is None, cannot launch browser.")
                    try:
                        logger.info("Launching headless Chromium browser...")
                        self._browser = await self._playwright_context.chromium.launch(headless=True)
                        logger.info("Browser launched successfully.")
                    except Exception as e:
                        logger.error(f"Failed to launch browser: {e}", exc_info=True)
                        await self.close_browser() # Attempt cleanup
                        raise BrowserManagerError(f"Failed to launch browser: {e}") from e
    ...
    async def open_page(self, url: str) -> Page:
        ...
        async with self._lock:
            ...
            try:
                ...
                logger.info(f"Successfully navigated to '{url}'. Title: '{page_title}'.")
                return self._page
            except Exception as e:
        ...
        async with self._lock:
            ...
            if self._playwright_context:
                 ...
                 try:
                      ...
                      logger.warning(f"Error stopping Playwright context: {e}", exc_info=True)
                 finally:
                      self._playwright_context = None
                      # Reset singleton instance if fully closed?
                      # BrowserManager._instance = None 
            
            logger.info(f"Browser close attempt finished. Browser closed: {browser_closed}, Playwright stopped: {playwright_stopped}")
