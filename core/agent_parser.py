...
def parse_llm_response(response: str, agent_logger: logging.Logger) -> Tuple[Optional[str], Optional[str], Optional[Dict[str, Any]]]:
    ...
    """
    agent_logger.debug(f"[Agent Parse DEBUG] Raw LLM Response (expecting ReAct Text):\n{response}")
    ...
    try:
        ...
        thought_match = re.search(r"^[ \t]*Thought:(.*)", response, re.MULTILINE | re.IGNORECASE)
        action_match = re.search(r"^[ \t]*Action:(.*)", response, re.MULTILINE | re.IGNORECASE)
        action_input_match = re.search(r"^[ \t]*Action Input:(.*)", response, re.MULTILINE | re.IGNORECASE | re.DOTALL) # DOTALL for multi-line JSON
        ...
        if action_match:
            ...
            agent_logger.debug(f"[Agent Parse DEBUG] Extracted Action: {action}")
             ...
             agent_logger.error(f"[Agent Parse ERROR] 'Action:' section not found in LLM response.\nResponse:\n{response}")
             # Return None tuple to signal critical parsing failure
             return None, None, None 

        if action_input_match:
            action_input_str = action_input_match.group(1).strip()
            agent_logger.debug(f"[Agent Parse DEBUG] Extracted Action Input String: {action_input_str}")
            ...
            if action_input_str:
                try:
                    ...
                    if action_input_str.startswith("```json"):
                        action_input_str = action_input_str.removeprefix("```json").removesuffix("```").strip()
             ...
             agent_logger.warning(f"[Agent Parse WARN] 'Action Input:' section not found for action '{action}'. Assuming empty input.")
        ...
        if action == "final_answer" and not action_input.get("answer"):
             agent_logger.warning(f"[Agent Parse WARN] 'final_answer' action detected but 'answer' key is missing in Action Input. Using thought or placeholder.")
             # Use thought as answer if available, otherwise use a placeholder
             final_answer_content = thought if thought else "(Final answer content not found)"
             action_input = {"answer": final_answer_content}


        agent_logger.info(f"[Agent Parse INFO] Text parsed successfully. Action: '{action}'")
        return thought, action, action_input

    except Exception as e:
         # Catch unexpected errors during regex or processing
         agent_logger.exception(f"[Agent Parse ERROR] Unexpected error during text parsing:")
         # Return None tuple to signal critical parsing failure
         return None, None, None
