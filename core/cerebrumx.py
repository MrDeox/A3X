...
from core.config import MAX_REACT_ITERATIONS # Example config import
from core.tools import get_tool_descriptions # <<< ADDED import
from core.tool_executor import execute_tool # <<< ADDED import
from core.llm_interface import call_llm # Ensure call_llm is imported
...
class CerebrumXAgent(ReactAgent): # Inheriting from ReactAgent for now
    ...
    def __init__(self, system_prompt: str, llm_url: Optional[str] = None):
        ...
        self.initial_perception = None # Store initial perception for potential replanning
    ...
    async def run_cerebrumx_cycle(self, initial_perception: Any) -> AsyncGenerator[Dict[str, Any], None]:
        """
        Executa o ciclo cognitivo completo do CerebrumX.
        Yields dictionaries representing each step of the cognitive cycle.
        """
        cerebrumx_logger.info(f"--- Iniciando Ciclo CerebrumX --- Perception inicial: {str(initial_perception)[:100]}...")
        self.initial_perception = initial_perception # Store for potential use in replanning
        ...
        cerebrumx_logger.info("--- Iniciando Execução do Plano com Simulação e Reflexão ---")
        ...
        async for execution_event in execute_plan_with_reflection(self, plan, context):
            ...
            # Handle replanning trigger if needed
                ...
                cerebrumx_logger.warning(f"Modification trigger received for step {step_index}. Reason: {reason}. Attempting to replan.")
                ...
                replanning_perception_data = {
                    ...
                    "processed": processed_perception.get("processed", "Original Objective Missing"), 
                    "modification_request": {
                        "failed_step_index": step_index,
                        "failed_step": plan[step_index] if step_index < len(plan) else "N/A",
                        "reason": reason,
                        "current_plan": plan
                    }
                }
                
                new_plan = await self._plan_hierarchically(replanning_perception_data, context) # Call planner again
                
                if new_plan != plan and new_plan: # Check if plan actually changed and is not empty
                    cerebrumx_logger.info(f"Re-planning successful. New plan generated with {len(new_plan)} steps. Restarting execution loop with new plan.")
                    ...
                    modification_needed = False # Reset flag
                    # Break the inner loop and let the outer method recall execute_plan_with_reflection? 
                    # Or restart the async for loop? Simpler to let it finish and restart the whole cycle? 
                    # For now, let's assume the execution generator handles the rest of the *old* plan steps as skipped due to modification
                    # And the next call to run_cerebrumx_cycle would use the updated state.
                    # Alternative: Restart the async for loop. This is complex.
                    # Let's just signal replan and let the current execution finish (likely skipping remaining steps).
                    # The next user interaction would then trigger a new cycle with potentially updated state/memory.
                    # OR: We could break here and restart the loop from the planning phase? Seems too complex for now.
                    # Simplest: Let the execution_logic finish (it skips modify steps now) and signal the replan happened.
                    pass # Continue processing yields from the finished execution_logic run
                else:
                    cerebrumx_logger.error("Re-planning failed or did not change the plan. Execution will continue skipping modified step.")
                    modification_needed = False # Reset flag, allow execution_logic to skip the step
    ...
    async def _retrieve_context(self, processed_perception: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if query and hasattr(self._memory, 'retrieve_relevant_context'):
            try:
                ...
                if isinstance(memory_result, dict) and "semantic_match" in memory_result:
                    semantic_match = memory_result["semantic_match"]
                     ...
                     cerebrumx_logger.info("Memory retrieval returned no relevant semantic context.")

            except Exception as e:
                cerebrumx_logger.exception(f"Error retrieving semantic context from memory for query '{query[:50]}...'")
                semantic_match = "Error during semantic memory retrieval."
        elif not hasattr(self._memory, 'retrieve_relevant_context'):
             cerebrumx_logger.warning("_memory object does not have 'retrieve_relevant_context' method.")
    ...
    async def _plan_hierarchically(self, perception: Dict[str, Any], context: Dict[str, Any]) -> List[str]:
        ...
        planner_result = {"status": "error", "data": {"message": "Planner execution failed."}} # Default
        try:
            ...
            cerebrumx_logger.exception("Exception occurred while executing the planner tool.")
        ...
        if planner_result.get("status") == "success":
            ...
            if isinstance(plan_list, list) and plan_list:
                cerebrumx_logger.info(f"Hierarchical plan generated successfully ({len(plan_list)} steps).")
                return plan_list
            else:
                cerebrumx_logger.warning("Planner skill succeeded but returned an empty or invalid plan list.")
                objective_str = str(perception.get("processed", "Fallback objective"))
                return [f"Address objective directly: '{objective_str[:50]}...'"]
        else:
            error_msg = planner_result.get("data", {}).get("message", "Unknown planning error")
            cerebrumx_logger.error(f"Hierarchical planner skill failed: {error_msg}")
            objective_str = str(perception.get("processed", "Fallback objective"))
            return [f"Address objective directly (planning failed): '{objective_str[:50]}...'"]

    async def _simulate_step(self, plan_step: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if tool_result.get("status") == "success":
                simulation_result = {
                    "simulated_outcome": tool_result.get("simulated_outcome", "Simulation success, but no outcome text provided."),
                    "confidence": tool_result.get("confidence", "N/A")
                }
            else:
                error_msg = tool_result.get("error_message", "Simulation skill failed without specific message.")
                cerebrumx_logger.error(f"Simulation skill failed for step '{plan_step[:50]}...': {error_msg}")
                simulation_result["simulated_outcome"] = f"Simulation Failed: {error_msg}"

        except Exception as e:
            cerebrumx_logger.exception(f"Exception calling simulate_step skill for step '{plan_step[:50]}...':")
            # Keep default error message
            simulation_result["simulated_outcome"] = f"Exception during simulation call: {e}"
    ...
    async def _reflect(self, perception: Dict[str, Any], plan: List[str], execution_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for i, result in enumerate(execution_results):
            step_description = plan[i] if i < len(plan) else f"Step {i+1} (description missing)"
        ...
        success_rate = (success_count / total_steps_attempted) * 100 if total_steps_attempted > 0 else 0
    ...
    async def _learn(self, reflection_output: Dict[str, Any]):
        ...
        overall_assessment = reflection_output.get("assessment", "No assessment provided.")
        
        if not learnings:
            cerebrumx_logger.info("No specific step learnings identified in this cycle.")
            # Optionally, still log the overall assessment
            log_entry = {"type": "cycle_summary", "assessment": overall_assessment}
        else:
            cerebrumx_logger.info(f"{len(learnings)} potential learning points identified.")
            ...
            for learning in learnings:
                 ...
                 if hasattr(self._memory, 'add_episodic_record'):
                     try:
                         ...
                         cerebrumx_logger.debug(f"Saved learning to episodic memory: {log_entry['type']} for step {log_entry['step_index']}")
                     except Exception as e:
                         cerebrumx_logger.exception(f"Failed to save learning point to episodic memory: {log_entry}")
                 else:
                     cerebrumx_logger.warning("Cannot save learning to memory: _memory object lacks 'add_episodic_record' method.")
