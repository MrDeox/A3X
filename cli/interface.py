...
try:
    from core.config import (
        MAX_HISTORY_TURNS, LOG_LEVEL, MEMORY_DB_PATH, LLM_PROVIDER,
    ...
    from core.tools import load_skills
...
def _parse_arguments():
    ...
    parser.add_argument("--model", help="Path para o arquivo do modelo GGUF a ser usado (sobrescreve config/default)")
    parser.add_argument("--gpu-layers", type=int, default=-1, help="Número de camadas a serem descarregadas na GPU (padrão: -1 = auto/todas)")
    parser.add_argument("--port", type=int, default=8080, help="Porta para iniciar o servidor LLM interno (padrão: 8080)")
    parser.add_argument("--no-server", action="store_true", help="Não iniciar um servidor LLM interno, usar a URL configurada (LLAMA_SERVER_URL)")
    return parser.parse_args()

def _initialize_agent(system_prompt: str, llm_url_override: Optional[str] = None) -> Optional[CerebrumXAgent]:
...
async def _run_interactive_mode(agent: CerebrumXAgent):
    ...
    console.print("[bold green]Modo Interativo CerebrumX.[/] Digite 'sair', 'exit' ou 'quit' para terminar.")
    ...
    if task_arg.strip().endswith(".json") and os.path.exists(task_arg):
        ...
        try:
            ...
            if isinstance(tasks_input, dict): # Single task
                ...
                raise TypeError("Task JSON must contain a single object or a list of objects.")

            # Process each task in the list
            for task_index, task in enumerate(tasks):
                logger.info(f"Processing task {task_index + 1}/{len(tasks)}: ID '{task.get('id', 'N/A')}'")
                ...
                if skill_name in ["open_url", "click_element", "fill_form_field", "get_page_content", "close_browser"]:
                ...
                result = {"status": "error", "message": f"Skill '{skill_name}' could not be executed."}
                try:
                    ...
                    if hasattr(skill_module, function_name):
                        ...
                        if asyncio.iscoroutinefunction(skill_function):
                            logger.info(f"Executing async skill '{skill_name}' with params: {params}")
                            result = await skill_function(**params) # Use await
                        else:
                            logger.info(f"Executing sync skill '{skill_name}' with params: {params}")
                            # Run sync function in executor to avoid blocking asyncio loop
                            loop = asyncio.get_event_loop()
                            result = await loop.run_in_executor(None, skill_function, **params)
                            
                        # Ensure result is a dict for consistent processing
                        if not isinstance(result, dict):
                             logger.warning(f"Skill '{skill_name}' did not return a dict. Wrapping output.")
                             result = {"status": "success", "message": str(result)}

                    else:
                        # Fallback logic (e.g., for class-based skills like AutoPublisher) might go here
                        # or raise an error.
                        logger.error(f"Function '{function_name}' not found in module '{module_path}'. Skill execution failed.")
                    ...
                    result = {"status": "error", "message": f"Skill module '{module_path}' not found."}
                except AttributeError as e:
                    logger.error(str(e))
                    result = {"status": "error", "message": str(e)}
                except Exception as e:
                    logger.exception(f"Error executing skill '{skill_name}' dynamically:")
                    result = {"status": "error", "message": f"Failed to execute skill: {e}"}

                # Display result
                logger.info(f"Task {task_index + 1} result: {result.get('status', 'N/A')}")
                console.print(Panel(
            ...
            console.print(f"[bold red]Erro:[/bold red] Arquivo JSON da tarefa não encontrado: '{task_arg}'")
        except json.JSONDecodeError:
            logger.error(f"Invalid JSON in task file: {task_arg}")
            console.print(f"[bold red]Erro:[/bold red] JSON inválido no arquivo da tarefa: '{task_arg}'")
            ...
            console.print(f"[bold red]Erro:[/bold red] Falha ao processar tarefas do JSON: {task_err}")
...
def _start_llama_server(
    ...
    gpu_layers: int,
    port: int,
    context_size: int = CONTEXT_SIZE # Use from config
    ...
    server_executable = os.path.join(project_root, "llama.cpp", "build", "bin", "llama-server")

    if not os.path.exists(server_executable):
        logger.error(f"llama-server executable not found at: {server_executable}")
        console.print(f"[bold red][Error][/] llama-server not found at expected location: {server_executable}. Please build llama.cpp.")
        return None, None
    ...
    logger.info(f"Starting internal llama-server on port {port} with model '{os.path.basename(model_path)}' ({gpu_layers} GPU layers...)")
    ...
    try:
        ...
        if _llama_server_process.poll() is not None:
             stderr_output = _llama_server_process.stderr.read().decode() if _llama_server_process.stderr else "No stderr output."
             logger.error(f"Internal llama-server failed to start. Exit code: {_llama_server_process.returncode}")
             logger.error(f"Server stderr:\n{stderr_output}")
             console.print(f"[bold red][Fatal Error][/] Internal llama-server failed to start. Check logs/stderr.")
        ...
        logger.info(f"Internal llama-server started successfully. PID: {_llama_server_process.pid}. URL: {server_url}")
        return _llama_server_process, server_url

    except Exception as e:
        logger.exception("Failed to start internal llama-server:")
        console.print(f"[bold red][Fatal Error][/] Could not start internal llama-server: {e}")
        return None, None
    ...
    if _llama_server_process and _llama_server_process.poll() is None:
        ...
        try:
            ...
            try:
                ...
                logger.warning("Internal llama-server did not terminate gracefully, killing...")
...
def run_cli():
    ...
    if args.stream_direct:
        ...
        try:
             asyncio.run(stream_direct_llm(args.stream_direct, llm_url_override=llm_server_url_to_use)) # Pass URL
             ...
             if server_process: _stop_llama_server()
        return
    ...
    if not agent:
        ...
        if server_process: _stop_llama_server() # Stop server if agent fails
        sys.exit(1)
    ...
    try:
        if args.task:
            ...
            logger.error("Inconsistent argument state. Cannot determine execution mode. Exiting.")
            sys.exit(1)
